        ...
        <div className="p-3 flex flex-row justify-center text-xs">     
          <div className="w-full">     
            ...
            <div className="h-26 text-center">
                ...
                <h6 className="text-sm font-bold mt-2">INFORME DE VENTAS</h6>
            ...
            <div className="border-1 border-gray-300 p-1 rounded-md m-1">                                
                ...
                <thead>
                    <tr>                                        
                    <th className="w-2/12 border border-slate-300 bg-gray-200">Fecha</th>                    
                    <th className="w-4/12 border border-slate-300 bg-gray-200">Origen</th>
                    <th className="w-3/12 border border-slate-300 bg-gray-200">Total Venta</th>
                    <th className="w-1/12 border border-slate-300 bg-gray-200">Gestión</th>
                    <th className="w-3/12 border border-slate-300 bg-gray-200">Usuario</th>                    
                    </tr>
                </thead>
                <tbody>
                    { props.pdata && (
                        props.pdata.map(item =>(
                            <tr key={item.id} className="hover:bg-gray-100 h-5">
                                <td className="border pl-1"><Moment format="DD/MM/YYYY">{item.fechaVenta}</Moment></td>                                
                                <td className="border pl-1">{item.origen}</td>                                  
                                <td className="border pl-1">{new Intl.NumberFormat('de-DE',{style: "currency",currency:"BOB"}).format(item.totalGeneral)}</td>
                                <td className="border pl-1">{item.gestion}</td>

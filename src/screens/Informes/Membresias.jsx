        ...
        <div className="p-3 flex flex-row justify-center text-xs">     
          <div className="w-full">     
            <div className="h-26 text-center">
                ...
                <h6 className="text-sm font-bold mt-2">INFORME MEMBRESIAS</h6>
            ...
            <div className="border-1 border-gray-300 p-1 rounded-md m-1">                                
                ...
                <thead>
                    ...
                    <th className="w-3/12 border border-slate-300 bg-gray-200">Nombres</th>
                    <th className="w-3/12 border border-slate-300 bg-gray-200">Paquetes</th>
                    <th className="w-1/12 border border-slate-300 bg-gray-200">F.Registro</th>
                    <th className="w-2/12 border border-slate-300 bg-gray-200">Monto</th>
                    <th className="w-2/12 border border-slate-300 bg-gray-200">Usuario</th>                    
                    </tr>
                </thead>
                <tbody>
                    { props.pdata && (
                        props.pdata.map(item =>(
                            <tr key={item.id} className="hover:bg-gray-100 h-5">
                                ...
                                <td className="border pl-1">{item.Cliente.nombres ? item.Cliente.nombres : 'n'}</td>                              
                                <td className="border pl-1">{item.Paquete.nombre ? item.Paquete.nombre : 'n'}</td>
                                <td className="border pl-1"><Moment format="DD/MM/YYYY">{item.createdAt}</Moment></td>                                                                                                  
                                <td className="border pl-1">{new Intl.NumberFormat('de-DE',{style: "currency",currency:"BOB"}).format(item.ingresos)}</td>
                                <td className="border pl-1">{item.Usuario.nombre ? item.Usuario.nombre : 'n'}</td>
                            </tr>
                        ))
                    )
                    }                    
           ...
           <div className="h-full w-auto mx-auto max-w-3xl flex-row justify-between ">
                <div className="h-11 flex flex-row p-1 ">                    
                    <div className="w-2/5">
                            <button 
                            onClick={handlePrint}
                            className="w-20 h-7 bg-red-500 rounded-md m-1 text-xs text-white">Imprimir
                            </button>                                                                   
                    </div>
